...
install_prereq() {
    ...
    # Modified: using zlib-ng-compat instead of zlib (modern, optimized drop-in replacement)
    $package_manager -Sy glibc libice libsm libx11 libxext libxi freetype2 libpng zlib-ng-compat lcms2 libgl libxcursor libxrandr glu alsa-lib fontconfig gnutls gsm libcups libdbus libexif libgphoto2 libldap libpulse libxcomposite libxinerama libxml2 libxslt libxxf86vm mpg123 nss-mdns ocl-icd openal openssl sane v4l-utils wine p7zip wget samba automake autoconf fakeroot make gcc --needed
    $package_manager -Sy lib32-glibc lib32-libice lib32-libsm lib32-libx11 lib32-libxext lib32-libxi lib32-freetype2 lib32-libpng lib32-zlib-ng-compat lib32-lcms2 lib32-libgl lib32-libxcursor lib32-libxrandr lib32-glu lib32-alsa-lib lib32-fontconfig lib32-libcups lib32-libdbus lib32-libexif lib32-libldap lib32-libpulse lib32-gnutls lib32-libxcomposite lib32-libxinerama lib32-libxml2 lib32-libxslt lib32-mpg123 lib32-nss-mdns lib32-openal lib32-openssl lib32-v4l-utils --needed
}
install_wine() {
    local wine_folder="/home/$USER/.wine-msoffice/wine"
    local script_dir="/home/tommy/Downloads/office-install.sh-main"
    local wine_archive="$script_dir/wine-9.7.zst"
    ...
    if check_reinstall "$wine_folder"; then
        ...
        # Modified: Use local wine file instead of downloading
        if [ ! -f "$wine_archive" ]; then
            echo "ERROR: Wine archive not found at $wine_archive"
            echo "Please ensure wine-9.7.zst is in the script directory."
            exit 1
        fi

        echo "Using local WINE 9.7 from $wine_archive..."
        echo "Extracting WINE 9.7..."
        mkdir -p "/home/$USER/.wine-msoffice/wine"
        tar --use-compress-program=unzstd -xf "$wine_archive" -C "$wine_folder"
...
install_ltsc() {
    local script_dir="/home/tommy/Downloads/office-install.sh-main"
    local office_archive="$script_dir/msoffice_ltsc.7z"
    ...
    if check_reinstall "/home/$USER/.wine-msoffice/LTSC"; then
        ...
        # Modified: Use local Office file instead of downloading
        if [ ! -f "$office_archive" ]; then
            echo "ERROR: Office archive not found at $office_archive"
            echo "Please ensure msoffice_ltsc.7z is in the script directory."
            exit 1
        fi

        echo "Using local Microsoft Office 2021 LTSC from $office_archive..."
        echo "Extracting Microsoft Office 2021 LTSC..."
        mkdir -p ~/.wine-msoffice
        7z x "$office_archive" -o/home/$USER/.wine-msoffice
        mv /home/$USER/.wine-msoffice/Microsoft_Office_365-3 /home/$USER/.wine-msoffice/LTSC
        echo "Microsoft Office 2021 LTSC has been installed to ~/.wine-msoffice/LTSC"
        register_ltsc_items
    else
        echo "Skipping reinstallation of Microsoft Office 2021 LTSC."
    fi
